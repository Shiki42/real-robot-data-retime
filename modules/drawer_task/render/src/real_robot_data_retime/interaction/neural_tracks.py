...
def segment_robots(frames, geometry, sam, sides=(0, 1)):
    ...
    for side in sides:
        ...
        reference_area = np.sum((geometry["masks"] == side + 1) & geometry["prompt_support"], axis=(1, 2))
        ...
        for a, b in stable_runs(missing, 15)[:2]:
            ...
            prompt = supported_robot_prompt(geometry["masks"][reset] == side + 1, geometry["prompt_support"][reset])
            seeds.append(
                dict(
                    frame=reset,
                    bbox=prompt["bbox"],
