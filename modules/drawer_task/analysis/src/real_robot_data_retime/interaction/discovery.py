...
def motion_and_grippers(frames, *, reference_mode="absolute"):
    """Background/color/connected geometry cues; no action or state inputs."""
    if reference_mode not in {"absolute", "darkening"}:
        raise ValueError("unknown motion reference mode")
    n, h, w = frames.shape[:3]
    bg = np.median(frames[: min(8, n)], axis=0).astype(np.uint8)
    background_gray = cv2.cvtColor(bg, cv2.COLOR_BGR2GRAY).astype(np.int16)
    ...
    for t, f in enumerate(frames):
        ...
        gray_prev = gray
        delta = (
            np.max(cv2.absdiff(f, bg), axis=-1)
            if reference_mode == "absolute"
            else background_gray - gray.astype(np.int16)
        )
