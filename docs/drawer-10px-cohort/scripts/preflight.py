...
R=Path('/home/coder/share/drawer-10px-cohort-20260912')
def run(ep):
...
if __name__=='__main__':
 ...
 with ProcessPoolExecutor(max_workers=3) as pool:
  for future in as_completed([pool.submit(run,ep) for ep in json.loads((R/'config.json').read_text())['source_indices']]):
   row=future.result();rows.append(row)
   (R/'preflight.json').write_text(json.dumps(dict(producer=producer_fingerprint(),scope='Planning only: no 2D or held-sphere rejection; drawer width .178m depth .20m height .07m; existing arm mesh checks retained; manual source60 boundary retained; not rendered',results=sorted(rows,key=lambda x:x['episode'])),indent=2))
