...
r=Path('/home/coder/share/drawer-stable-cohort-20260912');c=json.loads((r/'config.json').read_text());p=json.loads((r/'preflight.json').read_text());assert len(p['results'])==len(c['source_indices']) and all(x['passed'] for x in p['results']);logs=r/'render-logs';logs.mkdir(exist_ok=False)
def run(ep):
