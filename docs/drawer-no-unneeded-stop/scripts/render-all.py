...
r=Path('/home/coder/share/drawer-no-unneeded-stop-20260915');c=json.loads((r/'config.json').read_text());p=json.loads((r/'preflight.json').read_text());assert len(p['results'])==len(c['source_indices']) and all(x['passed'] for x in p['results']);logs=r/'render-logs';logs.mkdir(exist_ok=False)
def run(ep):
...
with ThreadPoolExecutor(max_workers=6) as pool:
 for f in as_completed([pool.submit(run,ep) for ep in c['source_indices'] if ep not in {26,27,29,30}]):
