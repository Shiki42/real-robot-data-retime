...
R=Path('/home/coder/share/drawer-no-unneeded-stop-20260915');c=json.loads((R/'config.json').read_text());out=Path(c['output']);ids=c['source_indices'];info=json.loads((out/'meta/info.json').read_text());rows=source_episodes(out);assert len(rows)==2*len(ids);results=[];total=0
for row in rows:
