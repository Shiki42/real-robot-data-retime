...
R=Path('/home/coder/share/drawer-uniform-dataset-20260912')
def validate(ep,c):
 ...
 for v in range(2):
  target=ep+v*87;receipt=json.loads((output/f'meta/retime_receipts/episode_{target:03d}.json').read_text());p=receipt['plan'];table=pq.read_table(output/f'data/chunk-000/file-{target:03d}.parquet')
