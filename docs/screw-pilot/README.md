...
准备完成按实际与命令 TCP 持续就位判断，避免把残余微调计为必须等待。微调沿原轨迹连续变速，双方接入原始协同区间。
...
模型分割来自 `approach-v2`，本条修复复用原分割。
边界和提示完整记录于 `config.json`，仍是人工核查的单条 episode 样例。

## 产物

工作目录：`/home/coder/share/screw-retiming-20260912/cohort/ep001`（coder a）。
严格裁剪后的原片为 2604 帧 / 86.8 秒；只删开头 4 帧，不补造终止静止。

| 样例 | u | 帧数 | 秒 |
| --- | ---: | ---: | ---: |
| 左臂先行 | 0.15 | 3209 | 106.967 |
| 中间时序 | 0.50 | 2448 | 81.600 |
| 右臂先行 | 0.85 | 3411 | 113.700 |

```bash
PYTHONPATH=src python -m real_robot_data_retime.screw \
  --source /home/coder/share/screw-retiming-20260912/raw \
  --work /home/coder/share/screw-retiming-20260912/cohort/ep001 \
  --config docs/screw-pilot/config.json --prepare
python scripts/build_screw_preview.py /home/coder/share/screw-retiming-20260912/cohort/ep001
```
...
- 39 项相关测试通过，覆盖晚到臂连续推进、短时间差不停车、右臂强制撤回及违规负例。
...
持续就位算法、阈值和新增 4 条示范的检查记录见 [多 episode 验证](../screw-cohort/README.md)。
